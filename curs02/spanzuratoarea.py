...
print(cuvant_de_inlocuit) # ne aviseaza o_o___o_ee
nr_vieti=3
while cuvant_de_inlocuit != cuvant:
    caracter_cerut = input("Alege o litera: ")
    print(caracter_cerut)
    if caracter_cerut in cuvant:
        lista_cuvant_de_inclocuit = list(cuvant_de_inlocuit)
        for i, v in enumerate(cuvant):
            if v == caracter_cerut:
                lista_cuvant_de_inclocuit[i] =  caracter_cerut
        cuvant_de_inlocuit = "," .join(lista_cuvant_de_inclocuit)
    else:
        nr_vieti -= 1
        if nr_vieti == 0:
